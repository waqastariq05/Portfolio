...
const ProjectDetail = (props) => {
    ...
    return (
        <>
            ...
            >
                <button className={`absolute top-[50%] -translate-y-[50%] text-white text-xl z-20 bg-black/90 left-1 p-2 rounded-r-sm opacity-50 group-hover:opacity-100 transition-all duration-300 cursor-pointer ${projectDetail.images.length === 1 ? "hidden" : "block"}`} onClick={() => { imageIndex === 0 ? setImageIndex(projectDetail.images.length - 1) : setImageIndex(imageIndex - 1) }} area-aria-label='left'>
                ...
                <div className="w-full lg:h-[60vh] md:h-64 sm:h-[90vh] h-72 overflow-hidden rounded-md shadow-md cursor-pointer relative inline-block border-4 border-purple-500 border-opacity-30">
                    <Image src={projectDetail.images[imageIndex].asset.url} width={500} height={500} className="w-full h-auto absolute top-0 right-0 left-[50%] translate-x-[-50%] rounded-sm max-w-none hover:top-[100%] hover:bottom-0 hover:translate-y-[-100%] hover:ranslate-x-[-50%] transition-all duration-1000 ease-linear" alt={projectDetail.name} priority sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" loading='eager' />
                </div>
                <button className={`absolute top-[50%] -translate-y-[50%] text-white text-xl z-20 bg-black/90 right-1 p-2 rounded-l-sm opacity-50 group-hover:opacity-100 transition-all duration-300 cursor-pointer ${projectDetail.images.length === 1 ? "hidden" : "block"}`}
                    onClick={() => { imageIndex < projectDetail.images.length - 1 ? setImageIndex(imageIndex + 1) : setImageIndex(0) }} area-aria-label='right'>
                    <FontAwesomeIcon icon={faChevronRight} />
                </button>
            </motion.div>

            {/* Details */}
            <div className="w-full md:w-1/2 lg:pl-10 md:pl-5 md:mt-0">
